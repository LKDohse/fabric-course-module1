...
public class WarturtleModel<T extends WarturtleEntity> extends SinglePartEntityModel<T> {
    ...
    private final float PiConstant =(float) Math.PI/180;
    ...
    @Override
    public void setAngles(WarturtleEntity entity, float limbSwing, float limbSwingAmount, float animationProgress, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);

        this.animateMovement(WarturtleAnimations.ANIM_WARTUTLE_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAnimationState, WarturtleAnimations.ANIM_WARTURTLE_IDLE, animationProgress, 1f);

        this.updateAnimation(entity.sittingTransitionAnimationState, WarturtleAnimations.ANIM_WARTURTLE_HIDE, animationProgress, 1f);
        this.updateAnimation(entity.sittingAnimationState, WarturtleAnimations.ANIM_WARTURTLE_SITTING, animationProgress, 1f);
        this.updateAnimation(entity.standingTransitionAnimationState, WarturtleAnimations.ANIM_WARTURTLE_EMERGE, animationProgress, 1f);
    ...
    private void setHeadAngles(float headYaw, float headPitch){
        ...
        this.head.yaw = headYaw * PiConstant;
        this.head.pitch = headPitch * PiConstant;
    }
