...
public class CrystallizerScreen extends HandledScreen<CrystallizerScreenHandler> {
    ...
    private static final Identifier CRYSTAL_TEXTURE =
            Identifier.of(MCCourseMod.MOD_ID , "textures/item/raw_fluorite.png");
    ...
    private void renderEnergyAreaTooltips(DrawContext context, int pMouseX, int pMouseY, int x, int y){
        if(isMouseAboveArea(pMouseX, pMouseY, x, y, 156, 9, 8, 48)) {
            context.drawTooltip(Screens.getTextRenderer(this), energyInfoArea.getTooltips(), Optional.empty(), pMouseX - x, pMouseY-y);
        }
    }
    ...
    protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
        ...
        renderProgressArrow(context, y, x);
    ...
    private void renderProgressArrow(DrawContext context, int y, int x) {
       if(handler.isCrafting()) {
